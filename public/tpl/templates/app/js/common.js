$(document).ready(function() {
	...
	$(".main-cats_item-wrap ul").after("<div class='mc_write_af'></div>");
	$('.main-cats_item-wrap > ul').hide();
	$(".main-cats_toggle").on('click', function(){
		$(this).parent().parent().find('.main-cats_item-wrap').removeClass('active');
		$(this).parent().parent().find('ul').hide();
		$(this).parent().parent().find('.mc_write_af').hide();
		$(this).parent().addClass('active');
		var context = $(this).parent();
		context.find('ul').slideToggle(1000);
		context.find('.mc_write_af').slideToggle(1500);
	});
	$(document).click(function (event) {
		if ($(event.target).closest('.main-cats_wrap').length == 0 && $(event.target).attr('id') != 'toggle-link') {
			$('.main-cats_item-wrap > ul:visible').slideToggle().removeClass('active');
			$('.main-cats_item-wrap').find('.mc_write_af:visible').slideToggle();
		}
	});
...
google.maps.event.addDomListener(document.getElementById('shake'), 'click', initialize);
*/
function initialize() {
    var goo = google.maps,
        mapOptions = {
