...
if($_FILES['upload']){
	if (($_FILES['upload'] == "none") OR (empty($_FILES['upload']['name'])) ){
		...
		move_uploaded_file($_FILES['upload']['tmp_name'], "../../../public/imgs/uploads/".$name);
		$full_path = 'http://localhost/frame-22-22/public/imgs/uploads/'.$name;
		$message = "Файл ".$_FILES['upload']['name']." загружен";
		$size=@getimagesize('../../../public/imgs/uploads/'.$name);
		if($size[0]<50 OR $size[1]<50){
			unlink('../../../public/imgs/uploads/'.$name);
